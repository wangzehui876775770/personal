...
for(var i=0;i<els.length;i++){
	els[i].style.backgroundImage='url(./images/beijing-'+i+'.png)'
}
// var aaa=function(){
// for(var i=0;i<els.length;i++){
...
for(var i=0;i<cc.length;i++){
	cc[i].style.width='600px';
	cc[i].index=i;
	cc[i].onmouseover=function(){
		cc[this.index].innerHTML='选择游戏';
	}
	cc[i].onmouseout=function(){
		cc[this.index].innerHTML=this.index;
	}
}
cc[0].onclick=function(){
	window.open('../poker/index.html')
}
cc[1].onclick=function(){
	window.open('../wuziqi/index.html')
}
cc[2].onclick=function(){
	window.open('../daziyouxi/index.html')
}
cc[4].onclick=function(){
	window.open('localhost:3000')
}
cc[3].onclick=function(){
	window.open('../tanchishse/index.html')
}
